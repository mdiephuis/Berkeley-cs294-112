...
def load_policy(filename):
    def read_layer(l):
        ...
        W, b = l['AffineLayer']['W'].astype(
            np.float32), l['AffineLayer']['b'].astype(np.float32)
    ...
    obsnorm_stdev = np.sqrt(np.maximum(
        0, obsnorm_meansq - np.square(obsnorm_mean)))
    ...
    def layers_forward(inp): return reduce(lambda x, fn: fn(x), [inp] + layers)
